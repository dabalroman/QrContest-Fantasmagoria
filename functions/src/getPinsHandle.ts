...
export const getPinsHandle = onCall(async (req): Promise<{ pins: PublicPin[] }> => {
    ...
    // Load-bearing anti-leak line: map field-by-field into PublicPin. NEVER `...doc.data()` — that would
    // hand every player every code (and every riddle answer) plus the full collectedBy finder map.
    const pins: PublicPin[] = snapshot.docs.map((doc) => {
